...
UNSEEN_TARGETS = ["Trimester", "Ecars"]
...
def load_predictions(res_dir):
    # Accept either:
    # 1) a folder containing submission.zip
    # 2) a folder containing predictions.txt
    # 3) a direct path to submission.zip
    ...
    if zip_path:
        extract_dir = os.path.join(os.path.dirname(zip_path), "extracted_submission")
...
def main():
    ...
    with open(scores_path, "w", encoding="utf-8") as f:
        ...
        unseen_df = gold_df[gold_df["target"].isin(UNSEEN_TARGETS)]

        if unseen_df.empty:
            raise ValueError(
                f"No rows found for UNSEEN_TARGETS={UNSEEN_TARGETS}. "
                f"Available targets are: {sorted(gold_df['target'].unique())}"
            )

        unseen_metrics = compute_metrics(
            unseen_df["stance"].tolist(),
            unseen_df["pred"].tolist()
        )
        ...
        f.write(f"Unseen_Overall_Favg2={unseen_metrics['Favg2']:.6f}\n")
        f.write(f"Unseen_Overall_Favg3={unseen_metrics['Favg3']:.6f}\n")
        f.write(f"Unseen_Overall_Accuracy={unseen_metrics['Accuracy']:.6f}\n")

        f.write(f"Test_Overall_Favg2={test_metrics['Favg2']:.6f}\n")
        f.write(f"Test_Overall_Favg3={test_metrics['Favg3']:.6f}\n")
        f.write(f"Test_Overall_Accuracy={test_metrics['Accuracy']:.6f}\n")
